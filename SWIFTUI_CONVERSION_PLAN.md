...
- Moved destructive/management actions (Add Folder, Rescan, Clear) out of the Library toolbar into a **Settings → Library** section to prevent accidental clicks; **Clear** now requires a confirmation alert (notes it removes all books + folders but leaves the audio files on disk). The Library empty state still offers a one-tap "Add Folder…"; per-folder removal stays on the source chips. `chooseLibraryFolder()` is a shared file-scope helper used by both.
